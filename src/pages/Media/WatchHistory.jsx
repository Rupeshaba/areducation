...
function NativeVideoStage({ content, onEnded, onBack, contentId }) {
  ...
  useEffect(() => {
    ...
    const onTime  = () => {
      ...
      if (time > 0 && duration > 0 && (time / duration) >= 0.95) {
        const completedKey = `ar_completed_${contentId || content.id || content._id}`
...
function YouTubeStage({ content, onBack, contentId, onEnded }) {
  ...
  // Mark as completed when YouTube video ends
  useEffect(() => {
    if (!started || !ytId) return
...
export default function MediaContent() {
  ...
  const markCompletedMutation = useMutation({
    mutationFn: ({ subjectId, courseId }) => api.post(`/user/progress/${contentId}/complete`, { subjectId, courseId }),
  })

  const handleEnded = useCallback(() => {
    if (user && content) {
      const subjId = content.subjectId || subjectId
      const courseIdVal = content.courseId || courseId
      markCompletedMutation.mutate({ subjectId: subjId, courseId: courseIdVal })
    }
  }, [user, markCompletedMutation, content, subjectId, courseId])
