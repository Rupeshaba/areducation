...
function HLSPlayer({ url, onEnded }) {
  ...
  // Fullscreen listener — also releases orientation lock when leaving fullscreen
  useEffect(() => {
    const onFS = () => {
  ...
  // Enter fullscreen + lock landscape orientation (best-effort; silently
  // no-ops on desktop or browsers that don't support orientation lock)
  const enterImmersive = useCallback(async () => {
    if (!containerRef.current?.requestFullscreen) {
      toast.error('DEBUG: requestFullscreen not supported on this element/browser')
    } else {
      try { await containerRef.current.requestFullscreen() }
      catch (e) { toast.error('DEBUG fullscreen failed: ' + e.name + ' - ' + e.message) }
    }
    if (!screen.orientation?.lock) {
      toast.error('DEBUG: screen.orientation.lock not supported on this browser')
    } else {
      try { await screen.orientation.lock('landscape') }
      catch (e) { toast.error('DEBUG orientation lock failed: ' + e.name + ' - ' + e.message) }
    }
  }, [])
...
function YouTubePlayer({ url, onEnded }) {
  ...
  const embedSrc = ytId
    ? `https://www.youtube.com/embed/${ytId}?autoplay=1&rel=0&modestbranding=1&enablejsapi=1&playsinline=1&origin=${encodeURIComponent(window.location.origin)}`
    : null
  ...
  // Click-to-play: enters fullscreen + locks landscape before the iframe
  // (which we can't otherwise intercept clicks inside, being cross-origin) loads
  const startPlayback = async () => {
    setStarted(true)
    if (!containerRef.current?.requestFullscreen) {
      toast.error('DEBUG: requestFullscreen not supported')
    } else {
      try { await containerRef.current.requestFullscreen() }
      catch (e) { toast.error('DEBUG fullscreen failed: ' + e.name + ' - ' + e.message) }
    }
    if (!screen.orientation?.lock) {
      toast.error('DEBUG: orientation.lock not supported')
    } else {
      try { await screen.orientation.lock('landscape') }
      catch (e) { toast.error('DEBUG orientation failed: ' + e.name + ' - ' + e.message) }
    }
  }
  ...
  return (
    <div ref={containerRef} className="relative w-full h-full bg-black overflow-hidden">
      {!started ? (
        <button onClick={startPlayback} className="absolute inset-0 w-full h-full flex items-center justify-center group">
          {thumbUrl && (
            <img src={thumbUrl} alt="" className="absolute inset-0 w-full h-full object-cover opacity-60" />
          )}
          <div className="absolute inset-0 bg-black/30" />
          <div className="relative w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-white/10 backdrop-blur-md border border-white/20 flex items-center justify-center group-hover:bg-primary-500/80 group-active:scale-90 transition-all">
            <Play size={30} className="text-white fill-current ml-1" />
          </div>
        </button>
      ) : (
        <>
...
// ─── Fullscreen PDF Viewer (portrait, no chrome besides the PDF itself) ─────
function FullscreenPDFViewer({ url, onBack }) {
  const [mode,    setMode]    = useState('google')
  const [loading, setLoading] = useState(true)

  const googleUrl = url ? `https://docs.google.com/viewer?url=${encodeURIComponent(url)}&embedded=true` : ''
  const directUrl = url ? `${url}#toolbar=0&navpanes=0` : ''
  ...
  return (
    <div className="fixed inset-0 bg-black z-50">
      ...
      <button
        onClick={onBack}
        className="fixed top-3 left-3 z-20 w-9 h-9 rounded-full bg-black/50 backdrop-blur-md border border-white/10 flex items-center justify-center text-white/80 hover:text-white hover:bg-black/70 transition-all"
      >
        <ArrowLeft size={17} />
      </button>
    </div>
...
export default function MediaContent() {
  ...
  if (isError) return (
    <div className="fixed inset-0 bg-black flex flex-col items-center justify-center gap-3 text-center px-6">
  ...
  if (content.type === 'pdf') {
    return <FullscreenPDFViewer url={content.url} onBack={() => navigate(backUrl)} />
  }
    ...
    <div className="fixed inset-0 bg-black z-50">
      {isYT
        ...
        : isHLS
          ...
          : <PlainVideoPlayer url={content.url} onEnded={handleEnded} />
      }
      <button
        onClick={() => navigate(backUrl)}
        className="fixed top-3 left-3 z-[60] w-9 h-9 rounded-full bg-black/50 backdrop-blur-md border border-white/10 flex items-center justify-center text-white/80 hover:text-white hover:bg-black/70 transition-all"
      >
        <ArrowLeft size={17} />
      </button>
    </div>
...
// ─── Plain MP4 fallback player — same auto fullscreen+landscape-on-play ─────
function PlainVideoPlayer({ url, onEnded }) {
  const videoRef     = useRef(null)
  const containerRef = useRef(null)
