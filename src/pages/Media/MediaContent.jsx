...
export function VideoPlayer() {
  ...
  useEffect(() => {
    ...
    const onTime = () => {
      ...
      if (v.currentTime - lastSaved.current >= 8) {
        ...
        saveProg.mutate({ position: Math.floor(v.currentTime), subjectId: content?.subjectId })
      }
    }
    ...
    const onEnd = () => { if (!doneFired) { setDoneFired(true)
        saveProg.mutate({ completed: true, position: 0, subjectId: content?.subjectId }) } }
  ...
  if (!content) return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] gap-3">
      ...
      <button onClick={() => navigate(-1)} className="text-indigo-400 text-sm font-medium hover:underline">Wapas jao</button>
    </div>
  )
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#0a0a14] to-[#14141e] py-3 px-2 sm:py-6 sm:px-6">
      <div className="max-w-3xl mx-auto w-full">
        {/* Back button */}
        <button onClick={() => navigate(-1)} className="flex items-center gap-1.5 text-white/60 hover:text-white text-sm mb-3 sm:mb-4 transition-colors">
          <ArrowLeft size={18} />
          <span>Back</span>
        </button>
        ...
        {/* Title */}
        <div className="mb-3">
          <h1 className="text-lg sm:text-2xl font-semibold text-white leading-tight">{content.title}</h1>
          <div className="flex flex-wrap items-center gap-2 mt-1.5">
            {content.duration && (
              <span className="text-xs text-white/50 bg-white/5 px-2 py-0.5 rounded-md">{fmtTime(content.duration)}</span>
            )}
            {isCompleted && (
        ...
        <div className="mt-6 flex flex-wrap gap-3 justify-center">
          {!isCompleted && (
            <button onClick={() => saveProg.mutate({ completed: true, position: 0, subjectId: content.subjectId })} className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-gradient-to-r from-emerald-500 to-emerald-600 text-white text-sm font-semibold hover:from-emerald-600 hover:to-emerald-700 transition-all active:scale-95">
              <CheckCircle size={16} />
...
export default VideoPlayerconst SPEEDS = [0.25, 0.5, 0.75, 1, 1.25, 1.5, 1.75, 2, 2.25, 2.5, 2.75, 3]
...
function InlinePDFViewer({ url, title }) {
  ...
  return (
    <div className="relative w-full mb-8">
      <div className="flex items-center justify-between mb-2 px-1">
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className={`text-xs px-2.5 py-1 rounded-lg transition-all ${mode === 'google' ? 'bg-primary-500/20 text-primary-300' : 'text-gray-500 hover:text-gray-300'}`}
          >Google Docs</button>
          <button
            onClick={() => { setMode('direct'); setLoading(true) }}
            className={`text-xs px-2.5 py-1 rounded-lg transition-all ${mode === 'direct' ? 'bg-primary-500/20 text-primary-300' : 'text-gray-500 hover:text-gray-300'}`}
          >Direct</button>
        </div>
        <a
          href={url}
      ...
      <div className="relative h-[75vh] w-full bg-dark-800 rounded-xl overflow-hidden border border-white/5">
        {loading && (
...
export default function MediaContent() {
  ...
  const isYT  = isYouTubeVideo(content.url)
  const isHLS = !isYT && (isHLSUrl(content.url) || content.type === 'hls')

  return (
    <div className="max-w-4xl mx-auto">
      <button onClick={() => navigate(backUrl)} className="text-primary-400 hover:text-primary-300 flex items-center gap-1 mb-4 text-sm">
        <ArrowLeft size={16} /> {chapterId ? 'Back to Chapter' : 'Back to Subject'}
      </button>

      <h1 className="text-2xl font-bold text-white mb-4">{content.title}</h1>
      {content.description && <p className="text-gray-400 text-sm mb-4">{content.description}</p>}

      {(content.type === 'video' || content.type === 'hls') && (
        <>
          <div className="w-full rounded-xl overflow-hidden bg-black shadow-2xl mb-2">
            {isYT
